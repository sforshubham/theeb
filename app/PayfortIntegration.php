...
class PayfortIntegration
{
    ...
    public function getRedirectionData($paymentMethod) {
        ...
        $postData = array(
            ...
            'return_url'          => $this->getUrl('/payment_response_route?r=processResponse'),
        );
    ...
    {
        $merchantReference = $this->generateMerchantReference();
        $returnUrl = $this->getUrl('/payment_response_route?r=merchantPageReturn');
        if(isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('/payment_response_route?r=merchantPageReturn&3ds=no');
        }
        $iframeParams              = array(
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
    ...
    public function processResponse()
    {
        ...
        if(!$success) {
            $p = $params;
            $p['error_msg'] = $reason;
            // $return_url = $this->getUrl('/payment_response_route?k=error&'.http_build_query($p));
            return ['status' => false, 'params' => $p];
        }
        else{
            // $return_url = $this->getUrl('/payment_response_route?k=success&'.http_build_query($params));
